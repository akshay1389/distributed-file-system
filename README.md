...
Developed and maintained by Akshay Basavalingaiah.
...
## Client End
- A client using the file system has six options upon startup, they can read, write, create and delete files, they can also find out which files are stored on the file system by using the 'show' command and find information on the syntax of the commands using the 'help' command.
...
- For example to read a file, the syntax is as simple as "read <file_name>" and everything else is handled under the hood.

## Servers
### Registry Server
- Keeps track of which server are online
- One json structure in database, entry structure:
```python
{
    'dir_name':
    'dir_port':
...
- Has two json file structures stored in the database:
  - Active nodes: Keeps track of the address of the file servers that are online, which files they are supporting and the load on that file server. Structure:
  ```python
  {
    'port':
    'file_names': []
...
## Features
### Replication
Replication of files across different file servers is mostly managed by the directory server. When a file server changes or deletes a file, it sends the details of those changes onto the directory server who sends them on again to every file server who supports the given file.

It is also important that when one of the file servers that supports the file goes down, access to the file is still available. This is done by letting the directory server know as soon as possible that the file server has gone down. This is managed by the registry server who constantly checks the status of the servers in the system.

### Locking
I chose to lock files when they are opened with write privileges. This means that when a client opens a file with write privileges, another client cannot access that file until the lock has been released by the first client.
...
## About the Developer
Akshay is a Backend Software Engineer with over 5 years of experience building scalable APIs, microservices, and distributed systems. With expertise in Python, Golang, and Java, he focuses on optimizing system performance and reliability in production environments. This project serves as a demonstration of distributed systems architecture, including load balancing, replication, and state management.